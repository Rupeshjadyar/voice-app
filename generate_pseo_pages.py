...
<nav class="glass-nav" style="position:fixed;top:0;width:100%;z-index:50;">
  <div style="max-width:1100px;margin:0 auto;padding:0 20px;display:flex;align-items:center;justify-content:space-between;height:60px;">
    ...
    <div class="nav-desktop" style="align-items:center;gap:6px;">
      ...
      <a href="/blog" class="nav-btn"><i class="fa-solid fa-newspaper"></i>Blog</a>
      <a href="/tts/" class="nav-btn nav-active"><i class="fa-solid fa-globe"></i>Languages</a>
...
<section style="max-width:860px;margin:0 auto;padding:0 18px 56px;">
  <div class="glass" style="padding:30px;">
    ...
    <div style="display:flex;gap:8px;margin-bottom:22px;background:var(--panel);border-radius:14px;padding:5px;border:1px solid var(--border);">
      ...
      <button class="tab-btn" id="tab-clone" onclick="switchTab('clone')" style="flex:1;">
        <i class="fa-solid fa-microphone-lines" style="margin-right:6px;color:var(--a1);"></i>Custom Voice Clone
      </button>
    </div>
    ...
    <div id="panel-clone" class="hidden">
      <div style="background:linear-gradient(135deg,rgba(59,158,255,0.06),rgba(124,95,230,0.06));border:1px solid rgba(59,158,255,0.18);border-radius:14px;padding:16px;margin-bottom:20px;font-size:.85rem;color:var(--txt2);">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="font-size:1.1rem;">🎙️</span>
          <strong style="color:var(--txt);font-family:'Syne',sans-serif;font-size:.92rem;">Instant AI Voice Cloning ({lang_name})</strong>
          <span id="cv-storage-badge" style="display:none;margin-left:auto;font-size:.7rem;padding:2px 8px;border-radius:20px;background:rgba(61,214,140,0.15);color:var(--ok);font-weight:700;border:1px solid rgba(61,214,140,0.3);">
            <i class="fa-solid fa-cloud-check" style="margin-right:4px;"></i>Profile Saved
          </span>
        </div>
        Record 5–15 seconds of your voice or upload a clean audio sample. Our AI will clone your vocal timbre and synthesize your {lang_name} text in your unique custom voice!
      </div>

      <!-- Voice Sample Input Options -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;margin-bottom:20px;">
        <!-- Card 1: Mic Record -->
        <div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:18px;display:flex;flex-direction:column;justify-content:space-between;">
          <div>
            <div style="font-size:.84rem;font-weight:700;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
              <i class="fa-solid fa-microphone" style="color:var(--a1);"></i> Option 1: Record Microphone
            </div>
            <p style="font-size:.76rem;color:var(--muted);margin-bottom:14px;">Speak clearly in any language for 5–15 seconds.</p>
          </div>
          <div>
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
              <span id="cv-timer" class="mono" style="font-size:.78rem;color:var(--muted);font-weight:700;">00:00 / 00:15</span>
              <span style="font-size:.72rem;color:var(--muted);">Min 5s recommended</span>
            </div>
            <button id="cv-record-btn" onclick="toggleVoiceRecording()" style="width:100%;padding:10px;border-radius:10px;border:none;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;font-family:'Syne',sans-serif;font-weight:700;font-size:.84rem;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:all .2s;">
              <i class="fa-solid fa-microphone"></i> Record Mic Sample
            </button>
          </div>
        </div>
        ...
        <div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:18px;display:flex;flex-direction:column;justify-content:space-between;">
          <div>
            <div style="font-size:.84rem;font-weight:700;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
              <i class="fa-solid fa-file-arrow-up" style="color:var(--a2);"></i> Option 2: Upload Audio File
            </div>
            <p style="font-size:.76rem;color:var(--muted);margin-bottom:14px;">Upload WAV, MP3, M4A, or OGG voice clip (max 10MB).</p>
          </div>
          <div>
            <input type="file" id="cv-file-input" accept="audio/*" onchange="handleVoiceFileUpload(event)" style="display:none;">
            <button onclick="document.getElementById('cv-file-input').click()" style="width:100%;padding:10px;border-radius:10px;border:1px solid var(--border);background:rgba(255,255,255,0.04);color:var(--txt);font-family:'Syne',sans-serif;font-weight:700;font-size:.84rem;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:all .2s;">
              <i class="fa-solid fa-cloud-arrow-up" style="color:var(--a2);"></i> Choose Audio File
            </button>
          </div>
        </div>
      </div>

      <!-- Sample Preview Bar (when audio is loaded) -->
      <div id="cv-preview-container" style="display:none;background:rgba(59,158,255,0.06);border:1px solid rgba(59,158,255,0.25);border-radius:14px;padding:14px 18px;margin-bottom:20px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:8px;">
          <div style="display:flex;align-items:center;gap:8px;">
            <span style="font-size:1.1rem;">🎧</span>
            <span style="font-size:.82rem;font-weight:700;color:var(--txt);">Your Reference Voice Sample</span>
            <span style="font-size:.7rem;padding:2px 8px;border-radius:12px;background:rgba(61,214,140,0.15);color:var(--ok);font-weight:600;">Active & Ready</span>
          </div>
          <button onclick="clearCustomVoiceSample()" style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.25);color:#ef4444;border-radius:8px;padding:4px 10px;font-size:.74rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:4px;">
            <i class="fa-solid fa-trash-can"></i> Remove Sample
          </button>
        </div>
        <audio id="cv-sample-player" controls style="width:100%;height:38px;"></audio>
      </div>

      <!-- Target Text for Cloned Voice -->
      <div style="margin-bottom:18px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
          <span class="slbl" style="margin-bottom:0;"><i class="fa-solid fa-pen-nib" style="margin-right:5px;color:var(--a1);"></i>Text to Speak in Cloned Voice ({lang_name})</span>
          <button onclick="loadCloneSample()" style="background:rgba(59,158,255,0.1);border:1px solid rgba(59,158,255,0.2);color:var(--a1);border-radius:8px;padding:4px 10px;font-size:.74rem;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;">
            <i class="fa-solid fa-magic-wand-sparkles" style="margin-right:4px;"></i>Sample {lang_name} Text
          </button>
        </div>
        <textarea id="cv-target-text" rows="4" placeholder="Type or paste the {lang_name} text you want your custom voice to speak... (up to 5000 chars)"></textarea>
      </div>

      <!-- Clone Options Grid -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-bottom:20px;">
        <div>
          <span class="slbl"><i class="fa-solid fa-tag" style="margin-right:5px;color:var(--a2);"></i>Voice Profile Name</span>
          <input type="text" id="cv-voice-name" value="My {lang_name} Voice" placeholder="Voice Name" style="width:100%;padding:10px 14px;border-radius:10px;border:1px solid var(--border);background:var(--panel);color:var(--txt);font-size:.88rem;outline:none;font-family:'DM Sans',sans-serif;">
        </div>
        <div>
          <span class="slbl"><i class="fa-solid fa-venus-mars" style="margin-right:5px;color:var(--a1);"></i>Vocal Range / Timbre</span>
          <select id="cv-gender-select">
            <option value="auto">⚡ Auto Detect (Recommended)</option>
            <option value="female">👩 Female Timbre Matching</option>
            <option value="male">👨 Male Timbre Matching</option>
          </select>
        </div>
      </div>

      <!-- Generate Button -->
      <button id="cv-clone-btn" class="btn-primary" onclick="doCloneVoice()" style="background:linear-gradient(135deg,var(--a1),var(--a2));">
        <span id="cv-clone-lbl"><i class="fa-solid fa-wand-magic-sparkles" style="margin-right:8px;"></i>Synthesize in My Cloned Voice &mdash; Free</span>
      </button>

      <!-- Cloned Voice Result Area -->
      <div id="cv-result-card" style="display:none;margin-top:22px;">
        <div style="background:rgba(255,255,255,0.015);border:1px solid var(--border);border-radius:16px;padding:18px 20px;margin-bottom:14px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
            <div style="display:flex;align-items:center;gap:10px;">
              <i class="fa-solid fa-circle-check" style="color:var(--ok);font-size:1.25rem;"></i>
              <div>
                <div style="font-weight:700;color:var(--ok);font-size:.9rem;font-family:'Syne',sans-serif;">Cloned Speech Ready!</div>
                <div id="cv-result-meta" style="font-size:.74rem;color:var(--muted);margin-top:1px;">AI Voice Clone &middot; {lang_name} &middot; Lossless Audio</div>
              </div>
            </div>
            <div class="badge-pill" style="margin:0;font-size:.7rem;background:rgba(124,95,230,0.12);color:var(--a2);border:1px solid rgba(124,95,230,0.25);">🎙️ Cloned Voice</div>
          </div>
          <audio id="cv-output-player" controls autoplay style="width:100%;margin-bottom:14px;"></audio>
          <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <button onclick="downloadClonedVoice('mp3')" style="flex:1;min-width:140px;background:rgba(61,214,140,0.12);border:1px solid rgba(61,214,140,0.28);color:var(--ok);border-radius:12px;padding:12px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif;font-size:.88rem;">
              <i class="fa-solid fa-download" style="margin-right:6px;"></i>Download MP3
            </button>
            <button onclick="downloadClonedVoice('wav')" style="flex:1;min-width:140px;background:rgba(124,95,230,0.12);border:1px solid rgba(124,95,230,0.28);color:var(--a2);border-radius:12px;padding:12px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif;font-size:.88rem;">
              <i class="fa-solid fa-download" style="margin-right:6px;"></i>Download WAV
            </button>
          </div>
        </div>
      </div>
...
<script>
  ...
  function switchTab(tab) {{
    ...
    const clone  = document.getElementById('panel-clone');
    const btnS   = document.getElementById('tab-single');
    const btnM   = document.getElementById('tab-multi');
    const btnC   = document.getElementById('tab-clone');
    if (tab === 'single') {{
      single.classList.remove('hidden'); multi.classList.add('hidden'); if(clone) clone.classList.add('hidden');
      btnS.classList.add('active');      btnM.classList.remove('active'); if(btnC) btnC.classList.remove('active');
    }} else if (tab === 'multi') {{
      single.classList.add('hidden'); multi.classList.remove('hidden'); if(clone) clone.classList.add('hidden');
      btnS.classList.remove('active'); btnM.classList.add('active');    if(btnC) btnC.classList.remove('active');
      ...
      single.classList.add('hidden'); multi.classList.add('hidden'); if(clone) clone.classList.remove('hidden');
      btnS.classList.remove('active'); btnM.classList.remove('active'); if(btnC) btnC.classList.add('active');
      initCustomVoiceStudio();
    }}
  }}
  ...
  /* ══════════════════════════════════════════════════════════════════════
     ✦ CUSTOM VOICE CLONING (Microphone & File Upload + Backend API)
     ══════════════════════════════════════════════════════════════════════ */
  let cvMediaRecorder = null;
  let cvAudioChunks = [];
  let cvRecordedBlob = null;
  let cvRecordingInterval = null;
  let cvRecordingSeconds = 0;
  let cvLastClonedAudio = null;
  let cvLastClonedFilename = null;

  function loadCloneSample() {{
    const el = document.getElementById('cv-target-text');
    if (el) {{
      el.value = SAMPLE_TEXT;
      showToast('Sample ' + LANG_NAME + ' text loaded!');
    }}
  }}

  async function convertBlobToCleanWav(blob) {{
    try {{
      const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
  ...
  function initCustomVoiceStudio() {{
    try {{
      const savedSample = localStorage.getItem('voicepro_custom_voice_sample');
      if (savedSample) {{
        fetch(savedSample)
          .then(res => res.blob())
          .then(async blob => {{
            cvRecordedBlob = await convertBlobToCleanWav(blob);
            const player = document.getElementById('cv-sample-player');
            if (player) player.src = savedSample;
            const preview = document.getElementById('cv-preview-container');
            if (preview) preview.style.display = 'block';
            const badge = document.getElementById('cv-storage-badge');
            if (badge) badge.style.display = 'inline-flex';
          }}).catch(err => console.warn('Restore voice error:', err));
      }}
    }} catch(e) {{
  ...
  async function toggleVoiceRecording() {{
    const btn = document.getElementById('cv-record-btn');
    const timer = document.getElementById('cv-timer');
    if (cvMediaRecorder && cvMediaRecorder.state === 'recording') {{
      cvMediaRecorder.stop();
      return;
    }}
    try {{
      const stream = await navigator.mediaDevices.getUserMedia({{ audio: true }});
      cvAudioChunks = [];
      cvMediaRecorder = new MediaRecorder(stream);
      cvMediaRecorder.ondataavailable = e => {{
        if (e.data.size > 0) cvAudioChunks.push(e.data);
      }};
      cvMediaRecorder.onstart = () => {{
        cvRecordingSeconds = 0;
        btn.innerHTML = '<i class="fa-solid fa-stop"></i> Stop Recording';
        btn.style.background = 'linear-gradient(135deg, #e11d48, #be123c)';
        if (timer) timer.textContent = '00:00 / 00:15';
        cvRecordingInterval = setInterval(() => {{
          cvRecordingSeconds++;
          const s = cvRecordingSeconds < 10 ? '0' + cvRecordingSeconds : cvRecordingSeconds;
          if (timer) timer.textContent = `00:${{s}} / 00:15`;
          if (cvRecordingSeconds >= 15) {{
            if (cvMediaRecorder && cvMediaRecorder.state === 'recording') {{
              cvMediaRecorder.stop();
            }}
          }}
        }}, 1000);
        showToast('Recording voice sample... Speak clearly! 🎙️');
      }};
      cvMediaRecorder.onstop = async () => {{
        clearInterval(cvRecordingInterval);
        stream.getTracks().forEach(track => track.stop());
        btn.innerHTML = '<i class="fa-solid fa-microphone"></i> Re-Record Mic';
        btn.style.background = 'linear-gradient(135deg, #ef4444, #dc2626)';
        const mime = cvMediaRecorder.mimeType || 'audio/webm';
        const rawBlob = new Blob(cvAudioChunks, {{ type: mime }});
        cvRecordedBlob = await convertBlobToCleanWav(rawBlob);
        const reader = new FileReader();
        reader.onloadend = () => {{
          const base64Data = reader.result;
          try {{
            localStorage.setItem('voicepro_custom_voice_sample', base64Data);
            localStorage.setItem('voicepro_custom_voice_time', new Date().toISOString());
            const badge = document.getElementById('cv-storage-badge');
            if (badge) badge.style.display = 'inline-flex';
          }} catch(e) {{
          ...
          const player = document.getElementById('cv-sample-player');
          if (player) player.src = base64Data;
          const preview = document.getElementById('cv-preview-container');
          if (preview) preview.style.display = 'block';
          showToast('Voice sample recorded & saved locally! 💾');
        }};
        reader.readAsDataURL(cvRecordedBlob);
      }};
      cvMediaRecorder.start(100);
    }} catch(err) {{
      console.error('Mic error:', err);
      showToast('Microphone access denied or not supported in this browser.', 'err');
    }}
  }}
  ...
  async function handleVoiceFileUpload(e) {{
    ...
    if (!file) return;
    cvRecordedBlob = await convertBlobToCleanWav(file);
    const reader = new FileReader();
    reader.onloadend = () => {{
      const base64Data = reader.result;
      try {{
      ...
      const preview = document.getElementById('cv-preview-container');
      if (preview) preview.style.display = 'block';
      showToast('Audio sample uploaded & saved! 📁');
    }};
    reader.readAsDataURL(cvRecordedBlob);
  }}

  function clearCustomVoiceSample() {{
    localStorage.removeItem('voicepro_custom_voice_sample');
    localStorage.removeItem('voicepro_custom_voice_name');
    localStorage.removeItem('voicepro_custom_voice_time');
    cvRecordedBlob = null;
    const preview = document.getElementById('cv-preview-container');
    ...
    const btn = document.getElementById('cv-record-btn');
    if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone"></i> Record Mic Sample';
    const timer = document.getElementById('cv-timer');
    if (timer) timer.textContent = '00:00 / 00:15';
    showToast('Voice profile deleted.', 'err');
  }}

  async function doCloneVoice() {{
    if (!cvRecordedBlob) {{
      showToast('Please record or upload your voice sample first! 🎙️', 'err');
      const btn = document.getElementById('cv-record-btn');
      if (btn) btn.focus();
      return;
    }}
    const textInput = document.getElementById('cv-target-text');
    const text = (textInput ? textInput.value : '').trim();
    if (!text) {{
      showToast('Please enter the ' + LANG_NAME + ' text you want your voice to speak! ✍️', 'err');
      if (textInput) textInput.focus();
      return;
    }}
    const voiceNameInput = document.getElementById('cv-voice-name');
    const voiceName = voiceNameInput ? voiceNameInput.value.trim() : ('My ' + LANG_NAME + ' Voice');
    const genderSel = document.getElementById('cv-gender-select');
    const gender = genderSel ? genderSel.value : 'auto';

    const btn = document.getElementById('cv-clone-btn');
    const lbl = document.getElementById('cv-clone-lbl');
    const resCard = document.getElementById('cv-result-card');

    btn.disabled = true;
    lbl.innerHTML = '<span class="spinner"></span><span style="margin-left:9px;">Cloning Voice & Synthesizing in ' + LANG_NAME + '…</span>';
    if (resCard) resCard.style.display = 'none';

    const fd = new FormData();
    fd.append('reference_audio', cvRecordedBlob, 'custom_voice_sample.wav');
    fd.append('text', text);
    fd.append('language', LANG_CODE);
    ...
    try {{
      const r = await fetch('/api/clone-voice', {{ method: 'POST', body: fd }});
      const d = await r.json();
      if (d.success) {{
        cvLastClonedAudio = d.audio_data;
        cvLastClonedFilename = d.filename || ('cloned_' + LANG_CODE + '_' + Date.now() + '.wav');
        const player = document.getElementById('cv-output-player');
        if (player) {{
          player.src = d.audio_data;
          player.play().catch(e => console.log('Autoplay blocked:', e));
        }}
        const meta = document.getElementById('cv-result-meta');
        if (meta) {{
          meta.textContent = `${{d.method || 'AI Voice Clone'}} · ${{voiceName}} · ${{LANG_NAME}}`;
        }}
        if (resCard) resCard.style.display = 'block';
        showToast('Custom Voice Cloned in ' + LANG_NAME + '! 🎉');
      }} else {{
        showToast(d.error || 'Voice cloning failed. Please retry.', 'err');
      }}
    }} catch(err) {{
      console.error('Clone API error:', err);
      showToast('Network error during voice cloning.', 'err');
    }} finally {{
      btn.disabled = false;
      lbl.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles" style="margin-right:8px;"></i>Synthesize in My Cloned Voice &mdash; Free';
    }}
  }}

  function downloadClonedVoice(type) {{
    if (!cvLastClonedAudio) {{
      showToast('Generate cloned audio first!', 'err');
      return;
    }}
    const ext = type === 'mp3' ? 'mp3' : 'wav';
    const fn = (cvLastClonedFilename || ('voicepro_cloned_' + LANG_CODE + '_' + Date.now())).replace(/\.[^.]+$/, '') + '.' + ext;
    const a = document.createElement('a');
    a.href = cvLastClonedAudio;
    a.download = fn;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    showToast('Downloading cloned ' + ext.toUpperCase() + '...');
  }}
