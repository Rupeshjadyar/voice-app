"""
Patch generate_pseo_pages.py: Replace PAGE_TEMPLATE with premium home-page-style UI.
Boundaries: before=17151, template_end_idx=69152, after_start=69153
"""
import sys

with open('generate_pseo_pages.py', 'r', encoding='utf-8') as f:
    content = f.read()

before = content[:17151]
after = content[69153:]

new_template = r'''PAGE_TEMPLATE = """<!DOCTYPE html>
<html lang="{html_lang}">
<head>
...
<style>
  ...
  *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
  ...
  @media(max-width:480px){{.glass{{border-radius:16px;}}}}
</style>
</head>
<body>
  ...
  <div class="glass" style="padding:30px;">
    ...
    <div id="panel-multi" class="hidden">
      <div style="background:rgba(59,158,255,0.05);border:1px solid rgba(59,158,255,0.15);border-radius:14px;padding:16px;margin-bottom:18px;font-size:.85rem;color:var(--txt2);">
        ...
        <strong>Multi-Voice Dialogue:</strong> Start lines with <code style="background:var(--panel);padding:2px 6px;border-radius:5px;">Speaker1:</code> and <code style="background:var(--panel);padding:2px 6px;border-radius:5px;">Speaker2:</code> &mdash; each gets its own voice!
      </div>
      <div style="margin-bottom:14px;">
        <span class="slbl">Dialogue Script</span>
        <textarea id="multi-txt" rows="8" placeholder="Speaker1: Hello! Welcome to VoicePro.&#10;Speaker2: Thank you! This is amazing.&#10;Speaker1: Generate {lang_name} audio free!&#10;Speaker2: Download as MP3 or WAV instantly!"></textarea>
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-bottom:18px;">
        <div>
...
<footer class="pro-footer">
  <div style="max-width:1100px;margin:0 auto;display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:40px;padding-bottom:44px;border-bottom:1px solid var(--border);">
    <div>
...
<script>
    ...
    for (let i = 0; i < lines.length; i++) {{
      ...
      try {{
        ...
        if (d.success) {{
          ...
          lineDiv.innerHTML = '<div style="font-size:.76rem;font-weight:700;color:' + color + ';margin-bottom:6px;font-family:Syne,sans-serif;">' + speakerLabel + '</div>' +
            '<div style="font-size:.9rem;color:var(--txt2);margin-bottom:10px;">' + text + '</div>' +
            '<audio src="' + d.audio_data + '" controls style="width:100%;height:36px;accent-color:' + color + ';"></audio>';
  ...
  function initCustomPlayer() {{
    const audio = document.getElementById('player'); if (!audio) return;
    audio.addEventListener('play',  () => {{ const ic = document.getElementById('play-pause-icon'); if(ic) ic.className='fa-solid fa-pause'; }});
    audio.addEventListener('pause', () => {{ const ic = document.getElementById('play-pause-icon'); if(ic) ic.className='fa-solid fa-play'; }});
    audio.addEventListener('timeupdate', () => {{
      const cur = audio.currentTime, dur = audio.duration || 0, pct = dur > 0 ? (cur/dur)*100 : 0;
      const prog = document.getElementById('player-progress');   if(prog) prog.style.width = pct + '%';
      const curT = document.getElementById('player-time-current'); if(curT) curT.textContent = formatTime(cur);
      const durT = document.getElementById('player-time-duration'); if(durT && dur) durT.textContent = formatTime(dur);
    }});
    audio.addEventListener('ended', () => {{
      const ic   = document.getElementById('play-pause-icon');     if(ic) ic.className='fa-solid fa-play';
      const prog = document.getElementById('player-progress');     if(prog) prog.style.width='0%';
      const curT = document.getElementById('player-time-current'); if(curT) curT.textContent='0:00';
    }});
  }}
...
new_content = before + new_template + after

with open('generate_pseo_pages.py', 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Done! New file size: {len(new_content)} chars")
print(f"New file lines: {new_content.count(chr(10))}")
